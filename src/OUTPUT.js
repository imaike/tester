        ...
        function setBackgroundColorFill() {
          ...
          for (var i =0, iLen = outputForDataViz.length; i<iLen; i++) {
            for (var j = 0, jLen = outputForDataViz[0].length; j < jLen; j++)  {
                ...
                if (matchingCountPercent <= cutoff) {isCaution = true;} else {isCaution = false;}
        ...
        for (var z = 0; z < outputForDataViz.length; z++) {
            ...
            var indexGroup2 = index2
                ...
                var pattern = svg.append("defs")
                	...
                	.append("rect")
                		.attr({ width:"2", height:"8", transform:"translate(0,0)", fill:consensusColor });

                var pattern2 = svg.append("pattern")
                    .append("pattern")
                		.attr({ id:"hash4_4b", width:"8", height:"8", patternUnits:"userSpaceOnUse", patternTransform:"rotate(135)"})
                	.append("rect")
                		.attr({ width:"2", height:"8", transform:"translate(0,0)", fill: matchCountColor  });

                var pattern3 = svg.append("pattern")
                    .append("pattern")
                		.attr({ id:"crosshatch", width:"8", height:"8", patternUnits:"userSpaceOnUse", patternTransform:"rotate(135)"})
                	.append("rect")
                		.attr({ width:"8", height:"8", transform:"translate(0,0)", fill:"white", stroke: overlapColor });
            ...
            indexGroup2
                ...
                .attr('fill', function (d) {
                    if (vizConfig.shouldShowBackgroundColor === true || vizConfig.shouldIndicateConsensus === true) {                     return d.displayFill;
                        } else {
                          return '#ffffff';
                      }})
            ...
            if (vizConfig.shouldShowMatchCounts === true) {
                indexGroup2
                    ...
                    .attr('font-size', cardFontSize)
                    .attr('x', function (d) {
                        return ((d.xVal * elementWidth) + (elementWidth * .97));
                    })
                    .attr('y', function (d) {
                        return ((d.yVal * elementHeight) + 38);
                    })
 ...
 */
 ...
 function pushFactorScoreCorrelationsToOutputArray(sheetNames, output) {
     ...
     var language = QAVnguage = QAV.getState("language");
